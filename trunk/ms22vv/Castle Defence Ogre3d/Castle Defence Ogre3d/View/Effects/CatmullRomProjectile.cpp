...
CatmullRomProjectile::CatmullRomProjectile(Ogre::Vector3 a_weaponPosition, Ogre::Quaternion a_weaponOrientation, Ogre::SceneManager *a_scenemgr, Ogre::Real a_fireId, Ogre::String a_weaponName, float a_offset, int a_distance)
{
	...
	m_spline->addPoint(a_weaponPosition + a_weaponOrientation * Ogre::Vector3(-a_offset,-a_offset,-a_distance * 0.2));
	m_spline->addPoint(a_weaponPosition + a_weaponOrientation * Ogre::Vector3(-a_offset,+a_offset,-a_distance * 0.4));
...
bool CatmullRomProjectile::Update(Ogre::Real a_timeSinceLastFrame)
{
	...
	if (m_time < (Ogre::Real)m_lifetime) { 
		m_projectileNode->setPosition( m_spline->interpolate(m_time/(Ogre::Real)m_lifetime) );
		return true;
	}
	// är bara för att jag vill hinna "fada" innan jag deletar.. 
	else if (m_time < m_lifetime*3) {
		return true;
	}
	else {
		return false;
...
{	
	...
	m_trail->setInitialWidth(0,4);
	m_trail->setWidthChange(0,10);
	m_trail->setTrailLength(600);
